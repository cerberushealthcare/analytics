...
class XmlRec {
  ...
  public function getText() {
    if (isset($this->_inner))
      return $this->_inner;
    else if (isset($this->_))
      return $this->_;
    else return get($this, '_value');
  }
  ...
  /**
   * @param string $fid array
   * @return string value of first element 
   */
  public function first($fid) {
    if (! empty($this->$fid))
      return reset($this->$fid);
  }
  //
  ...
  static function parse($xml) {
    ...
    if (! $dom->loadXml($xml))
      throw new XmlParseException('XML Parse Exception');
  ...
  protected static function parseNode($root) {
    ...
    if ($root->hasAttributes()) {
      ...
      foreach ($root->attributes as $name=>$node) {
        if ($node->nodeName != $name)
          $name = static::fixParsedAttrName($node->nodeName);
        $fid = "_$name";
        $rec->$fid = $node->nodeValue;
      }
    }
    $children = $root->childNodes;
    if ($rec || ($children && $children->length)) {
      if ($rec == null)
        $rec = new XmlRec();
      if (static::isMixedText($children)) {
        $rec->_inners = array();
        foreach ($children as $child) {
          $name = static::fixParsedName($child->nodeName);
          if ($name == '#text') 
            $rec->_inners[] = array($name => $child->nodeValue);
          else
            $rec->_inners[] = array($name => static::parseNode($child));
        }
        $rec->_inner = $root->textContent;
        return $rec;
      }
  ...
  private static function isMixedText($children) {
  ...
  private static function fixParsedAttrName($name) {
    $a = explode(':', $name);
    return implode('_', $a); 
  }
  private static function fixParsedName($name) {
    $a = explode(':', $name);
    return end($a); 
  }
  ...
  public static function buildXml($e, $formatted = false, $rootTagName = null, $includeEmpties = false, $noHyphenate = false) {
    ...
    $root = ($rootTagName != null) ? 
      ...
      null;
    foreach (get_xml_vars($e) as $key => $value) {
  ...
  private static function createNodeHtml($key, $value, &$parent) {
    ...
    if (substr($key, 0, 1) == '_')  {
      ...
      if ($value === null) {
        ...
        $node = self::$dom->createElement($key);
        foreach (get_xml_vars($value) as $key => $value) {
...
function get_xml_vars($o) { /*external function to restrict get_object_vars to public vars only*/

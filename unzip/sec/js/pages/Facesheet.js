...
var Facesheet = page = {
  ...
  pPopHm:function(pcid) {
    IpcHmSelector.pop(function(rec) {
      alert('hi, rec=' + rec.Ipc.name);
    });
    //HmTile.pop(pcid);
  },
  pPopProcs:function() {
  ...
  _resizeDivs:function() {
    syncHeights([MedTile.table, DiagTile.table], 100);
...
MedTile = {
    ...
    return MedTile = Html.Tile.create(container.clean()).extend(function(self) {
      return {
        ...
        init:function() {
          ...
          self.cmdtile = Html.Tile.create(self, 'cmdtile').add(Html.Anchor.create('qicon erxi', 'Prescribe...', page.pNewCrop)).hide();
        },
        load:function(fs) {
  ...
  Table:{
    create:function(container) {
      return Html.TableLoader.create(container, 'fsb', 'fstab noscroll').extend(function(self) {
        return {
          init:function() {
            self.tbody().tr().td().html('&nbsp;');
          },
...
DocTile = {
  create:function(container) {
    ...
    return DocTile = Html.Tile.create(container.clean()).extend(function(self) {
      return {
        init:function() {
          self.table = My.Table.create(self).bubble('onselect', self.table_onselect);
        },
        load:function(fs) {
...
DocHistoryPop = {
  ...
  create:function() {
    ...
    return self.aug({
      init:function() {
        self.table = DocHistoryTable.create(self.content).bubble('onload', self.table_onload);
...
DiagTile = {
  create:function(container) {
    ...
    return DiagTile = Html.Tile.create(container.clean()).extend(function(self) {
      return {
        ...
        init:function() {
          self.table = My.Table.create(self).bubble('onselect', self.table_onselect).bubble('ondraw', self);
          self.cmdtile = Html.Tile.create(self, 'cmdtile').hide();
        },
        load:function(fs) {
  ...
  Table:{
    create:function(container) {
      return Html.TableLoader.create(container, 'fsy', 'fstab noscroll').extend(function(self) {
        return {
          init:function() {
            self.tbody().tr().td().html('&nbsp;');
          },

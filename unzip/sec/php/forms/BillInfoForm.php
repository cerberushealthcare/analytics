...
class BillInfoForm extends Form {
  ...
  public function setMode() {
    ...
    if ($login->User->isOnTrial()) {
      ...
      if ($login->expireReason == User0::EXPIRE_CARD_EXPIRED) {
        $this->modeCaption = "<p class='red'>" . $login->expireReason . "</p>";
      } else {
        $this->modeCaption = "";
      }
  ...
  public function setFromDatabase($dto) {
    ...
    $this->card_number = MyCrypt_Auto::decrypt($dto->cardNumber);
  ...
  public function sendMail() {
    ...
    if (! $login->isProdEnv()) 
      return;
    $user = RegistrationDao::getUserInfo($login->uid);
    $to = $user["email"];

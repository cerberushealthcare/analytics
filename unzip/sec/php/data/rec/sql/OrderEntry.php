...
class OrderEntry {
  ...
  static function /*TrackItemStub[]*/getActiveItems($clientId) {
    $c = TrackItemStub::asActiveCriteria($clientId);
  ...
  static function /*TrackItemList[]*/getOpenItems($clientId = null) {
    ...
    $recs = TrackItem::fetchAllBy($c, new RecSort('clientId', 'trackDesc', 'dueDate'), 2000);  
    $recs = RecSort::sort(static::omitUnschedRepeats($recs), 'trackCat', 'ClientStub.name', 'trackDesc', 'dueDate');
    return $recs;
  }
  ...
  static function /*TrackItemList[]*/getUnschedItems($clientId = null) {
    ...
    $recs = TrackItem::fetchAllBy($c, new RecSort('clientId', 'trackDesc', 'dueDate'), 2000);  
    $recs = RecSort::sort(static::omitUnschedRepeats($recs), '-priority', 'dueDate', 'ClientStub.name', 'trackDesc');
    return $recs;
  }
  ...
  static function /*TrackItem_Referrals[]*/getActiveReferrals($cid) {
    $recs = TrackItem_Referrals::fetchAllActive($cid);
    return $recs;
  }
  static function /*TrackItem_Followups[]*/getActiveFollowups($cid) {
    $recs = TrackItem_Followups::fetchAllActive($cid);
    return $recs;
  }
  static function /*TrackItem_Tests[]*/getFutureTests($cid) {
    $recs = TrackItem_Tests::fetchAllFuture($cid);
    return $recs;
  }
  static function /*TrackItem_Tests[]*/getPendingTests($cid) {
    $recs = TrackItem_Tests::fetchAllPending($cid);
    return $recs;
  }
...
class TrackItem extends TrackItemRec {
  ...
  public function toJsonObject(&$o) {
    ...
    if (isset($o->Event) && isset($o->Event->until)) {
  ...
  public function getJsonFilters() {
    return array(
      ...
      'drawnDate' => JsonFilter::editableDateTime(),
      '_drawnDate' => JsonFilter::editableTimeInformal('drawnDate'),
  ...
  public function isReferral() {
    return $this->trackCat == static::TCAT_REFER && $this->cptCode != Ipcs::FollowupVisit;      
  }
  public function isLab() {
    return $this->trackCat == static::TCAT_LAB;
  }
  public function isRadiology() {
    return $this->trackCat == static::TCAT_RADIO;
  }
  public function save() {
    global $login;
    logit_r($this, 'save 1919');
    if (! empty($this->Event)) {
      $this->Event->save();
  ...
  protected function saveTriggers() {
    if ($this->isLab()) {
      Proc_OrderLab::record_fromTrackItem($this);
      Proc_CpoeLab::record_fromTrackItem($this);
    }
    if ($this->isRadiology()) {
      Proc_OrderRadio::record_fromTrackItem($this);
      Proc_CpoeRadio::record_fromTrackItem($this);
    }
    if ($this->isReferral()) {
      Proc_Referral::record_fromTrackItem($this);
    }
  }
  ...
  private function determineStatus() {
    ...
    if ($this->schedDate || $this->selfRefer || $this->drawnDate)
  ...
  static function fromUi($o) {
    $me = new TrackItem($o);
    if (empty($me->schedDate) && empty($me->drawnDate) && empty($me->closedDate)) {// only save undated updates to repeats
...
class TrackItem_Ccda extends TrackItemRec {
  //
  public $trackItemId;
  public $userGroupId;
  public $clientId;
  ...
  static function fetchAllActive($cid) {
    $c = static::asActiveCriteria($cid);
    return static::fetchAllBy($c);
  }
}
class TrackItem_Referrals extends TrackItem_Ccda {
  //
  static function asActiveCriteria($cid) {
    $c = parent::asActiveCriteria($cid);
    $c->cptCode = CriteriaValues::_or(CriteriaValue::isNull(), CriteriaValue::notEquals(Ipcs::FollowupVisit));
    $c->Provider = Provider::asRequiredJoin('schedWith');
    $c->Address = Address::asOptionalJoin('schedLoc');
    $c->OrderBy = UserStub::asOptionalJoin('orderBy');
    return $c;
  }
}
class TrackItem_Followups extends TrackItem_Ccda {
  //
  static function asActiveCriteria($cid) {
    $c = parent::asActiveCriteria($cid);
    $c->schedDate = CriteriaValue::isNotNull();
    $c->cptCode = Ipcs::FollowupVisit;
    $c->Provider = Provider::asRequiredJoin('schedWith');
    $c->Address = Address::asOptionalJoin('schedLoc');
    return $c;
  }
}
class TrackItem_Tests extends TrackItem_Ccda {
  //
  static function fetchAllFuture($cid) {
    $c = static::asActiveCriteria($cid);
    $c->drawnDate = CriteriaValue::isNull();
    return static::fetchAllBy($c);
  }
  static function fetchAllPending($cid) {
    $c = static::asActiveCriteria($cid);
    $c->drawnDate = CriteriaValue::isNotNull();
    return static::fetchAllBy($c);
  }
  static function asActiveCriteria($cid) {
    $c = parent::asActiveCriteria($cid);
    $c->Ipc = Ipc::asRequired_noAdmin('cptCode');
    $c->OrderBy = UserStub::asOptionalJoin('orderBy');
    return $c;
  }
} 

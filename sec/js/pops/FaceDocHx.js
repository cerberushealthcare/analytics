...
FaceDocHx = {
  ...
  pop:function(fs, tab, zoom, callback) {
    ...
    var self = this;
    Includer.get(Includer.HTML_FACE_DOC_HX, function() {
      ...
      if (fs.client) {
        Pop.setCaption("fsp-his-cap-text", fs.client.name + " - Documentation/Visits");
      }
      self._load();
    });
  },
  fpTiani:function() {
    TianiDocs.pop(this.fs.client.clientId);
  },
  fpClose:function() {
    Pop.close();
    if (this.changed) 
  ...
  _load:function() {
    ...
    if (hist && hist.all) {
      ...
      if (this.zoom) {
        Pop.zoom(FaceDocHx._POP);
      } else {
        Pop.show(FaceDocHx._POP);
      }
    }
  },
    ...
    if (e) {
      if (e.type == 0) {
        ...
        if (sess) {
          ...
          tp.createTdAppend(null, FaceUi.createSessionAnchor(fs, sess.id, true));

...
class ErxStatus extends Rec {
  ...
  public static function fromNewCrop($status, $nc) {
    ...
    if ($status == ErxStatus::STATUS_STAFF_PROC) {
      $rec->staff = "$nc->StaffFirstName $nc->StaffLastName";
    } else if ($rec->UserStub_Doctor == null) {
      $rec->UserStub_Doctor = new UserStub();
      $rec->UserStub_Doctor->name = "$nc->DoctorFirstName $nc->DoctorLastName";
    }
    return $rec;
  }
  public function fetchStubs() {
    if (is_numeric($this->clientId)) 
      $this->ClientStub = ClientStub::fetchByEmrId($this->clientId);
    if ($this->doctorId) 

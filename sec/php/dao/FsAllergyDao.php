...
class FsAllergyDao {
  ...
  public static function rebuildNewCropAllergies($clientId, $current) {
    ...
    $faceRecs = JDataAllergy::getActiveNewCropAllergies($clientId);
    $now = nowNoQuotes();
    foreach ($allergies as $agent => $allergy) {
      if (! isset($faceRecs[$agent])) {  
        $face = JDataAllergy::copy($allergy);
        $face->sessionId = null;
        $face->index = null;
        $face->date = $now;
        $face->active = true;
    ...
    foreach ($faceRecs as $agent => $face) {
      if (! isset($allergies[$agent])) {
        $face->active = false;
        JDataAllergy::save($face, true);
      }
    }
  }
	/**

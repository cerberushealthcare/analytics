...
class DG1 extends HL7Segment {
  ...
  static function from($fs) {
    if ($fs->Diagnoses) {
      $mes = array();
      $seq = 0;
      foreach ($fs->Diagnoses as $diag) 
        $mes[] = static::fromDiag($fs, $diag, $seq);
      return $mes;
    }
  }
  static function fromDiag($fs, $diag, &$seq) {
    $me = self::asEmpty();
    $me->seq = $seq++;
    ...
    if ($diag->icd)
      $me->codingMethod = 'I9';
    $me->desc = $diag->text;
    $me->date = TS::fromDate($diag->date);
    $me->type = IS_DiagnosisType::asFinal();
    return $me;
  }
}

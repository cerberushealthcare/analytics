...
class UserLogin extends UserRec implements NoAuthenticate {
  ...
  static function fetchByUidTest($uid) {
  	Logger::debug('fetchByUidTest: getting user from uid' . $uid);
    $c = static::asCriteria($uid);
	...
	$result = static::fetchOneBy_Test($sql, $c);
	Logger::debug('fetchByUidTest with uid ' . $uid . ': Returning a(n) ' . gettype($result) . ' ' . print_r($result));
	//Logger::debug('fetchByUidTest: result 0 is ' . var_dump(get_object_vars($result)));
	return $result;
  }

  ...
  Table:{
    create:function(parent) {
      ...
      return self.aug({
        ...
        edit:function(rec) {
          DrugClassEntry.pop(rec)
            .bubble('onsave', self.update)
            .bubble('ondelete', self.update);
        },
        ...
        add:function(rec, tr) {
          ...
          Array.forEach(rec.DrugSubclasses, function(sub) {
            sub.DrugClass = rec;
            self.tbody().tr(sub).check().select(rec.name + ': ' + sub.name, sub, self.sub_onselect, 'c2');;
            ...
            self.tbody().tr().check().td(Html.AnchorAction.asNew('Add drug...').set('onclick', function(){self.drug_onadd(sub)}), 'c3');
          })
          self.tbody().tr().check().td(Html.AnchorAction.asNew(rec.name + ': Add subclass...').set('onclick', function(){self.sub_onadd(rec)}), 'c2');
        },
        onselect:function(rec) {
          self.edit(rec);
        },
        sub_onselect:function(rec) {
          DrugSubclassEntry.pop(rec)
            .bubble('onsave', self.update)
        ...
        drug_onselect:function(rec) {
          DrugNameEntry.pop(rec)
            .bubble('onsave', self.update)
            .bubble('ondelete', self.load.curry(null));
        },
...
DrugNameEntry = {
  ...
  create:function() {
    ...
    return self.aug({
      ...
      save:function(rec, callback_rec) {
        if (rec.DrugSubclass) {
          rec.DrugSubclass.DrugClass = null;
          rec.DrugSubclass.DrugNames = null;
        }

...
class DomData {
  ...
  private static function buildXml($e, $rootTagName, $includeEmpties) {
    ...
    if (count($required) > 0)
      throw new DomDataRequiredException($required, $e);
    if ($root) 
...
class DomDataRequiredException extends Exception {
   ...
   */
  public $required;
  public function __construct($required, $e) {
    $friendlies = $e::$FRIENDLY_NAMES;
    foreach ($required as &$name) 
      if (isset($friendlies[$name]))
        $name = $friendlies[$name];

...
class TPar extends ParRec implements ReadOnly {
  ...
  static function fetchInject($sid, $uid, $userId) {
    $c = new static();
    $c->uid = $uid;
    $c->sectionId = $sid;
    $c->current = 1;
...
class TPars {
  ...
  static function /*TPar[]*/fetchAll($pid, $userId) {  
    ...
    static::append($recs, $rec, $userId, $rec->Section->uid);
    return array_values($recs);
  }
  protected static function append(&$recs, $par, $userId, $suid) {
    if (! isset($recs[$par->parId])) {
      $recs[$par->parId] = $par;
      $pars = static::fetchInjectsFor($par, $suid, $userId);
      foreach ($pars as $par)
        static::append($recs, $par, $userId, $suid);
    }
  }
  protected function fetchInjectsFor($par, $suid, $userId) {
    $uids = TQuestion::getAllInjectUids($par->Questions, $suid);
    $pars = array();
    foreach ($uids as $uid)
      $pars[] = TPar::fetchInject($par->sectionId, $uid, $userId);
    return $pars;
  }
}
//

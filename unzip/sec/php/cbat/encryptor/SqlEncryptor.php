...
class SqlEncryptor {
  ...
  static function getAllSets($ugid) {
    return array_merge(
      ...
      static::getSets('Session', $ugid),
      static::getSets('Proc', $ugid),
      static::getSets('DataSync', $ugid),
      static::getSets('AuditRec', $ugid),
      ...
      static::getSets('Med', $ugid),
      static::getSets('Immun', $ugid),
      static::getSets('Diagnosis', $ugid),
      static::getSets('Allergy', $ugid),
      static::getSets('ScanIndex', $ugid),
      static::getSets('Vitals', $ugid),
      static::getSets('Sched', $ugid));
  }
  //
  protected static function getSets($name, $ugid, $limit = 5000) {
    $sets = array();
  ...
  protected static function appendSets(&$sets, $name, $ugid, $limit, $index = 1, $startPk = null) {
    ...
    $sets[] = EncryptSet::from($lset, $name, $ugid, $index);
    if ($lset->nextPk)
...
class SqlFile_E extends TextFile {
  //
  static function from(/*EncryptSet*/$set) {

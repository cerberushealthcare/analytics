...
class CmsReports {
   ...
   */
  static function getNQF0043($from, $to, $userId) {
    return CmsReport_NQF0043::from($from, $to, $userId);
  }
  /**
   * NQF 0034
   * Colorectal Cancer Screening
   */
  static function getNQF0034($from, $to, $userId) {
    return CmsReport_NQF0034::from($from, $to, $userId);
  }
  /**
   * NQF 0027
   * Smoking and Tobacco Use Cessation, Medical assistance
   */
  static function getNQF0027($from, $to, $userId) {
    return array(
      CmsReport_NQF0027_N1::from($from, $to, $userId),
      CmsReport_NQF0027_N2::from($from, $to, $userId));
  }
  /**
   * NQF 0038
   * Childhood Immunization Status
   */
  static function getNQF0038($from, $to, $userId) {
    return array(
      CmsReport_NQF0038_N1::from($from, $to, $userId),
      CmsReport_NQF0038_N2::from($from, $to, $userId),
      CmsReport_NQF0038_N3::from($from, $to, $userId),
      CmsReport_NQF0038_N4::from($from, $to, $userId),
      CmsReport_NQF0038_N5::from($from, $to, $userId),
      CmsReport_NQF0038_N6::from($from, $to, $userId),
      CmsReport_NQF0038_N7::from($from, $to, $userId),
      CmsReport_NQF0038_N8::from($from, $to, $userId),
      CmsReport_NQF0038_N9::from($from, $to, $userId),
      CmsReport_NQF0038_N10::from($from, $to, $userId),
      CmsReport_NQF0038_N11::from($from, $to, $userId),
      CmsReport_NQF0038_N12::from($from, $to, $userId));
  }
  /**
   * NQF 0013
   * Hypertension: Blood Pressure Measurement
   */
  static function getNQF0013($from, $to, $userId) {
    return CmsReport_NQF0013::from($from, $to, $userId);
  }
  /**
   * NQF 0421
   * Adult Weight Screening and Follow-Up
   */
  static function getNQF0421($from, $to, $userId) {
    return array(
      CmsReport_NQF0421_P1::from($from, $to, $userId),
      CmsReport_NQF0421_P2::from($from, $to, $userId));
  }
  /**
   * NQF 0041
   * Preventive Care and Screening: Influenza Immunization for Patients >= 50 Years Old
   */
  static function getNQF0041($from, $to, $userId) {
    return CmsReport_NQF0041::from($from, $to, $userId);
  }
  /**
   * NQF 0028a and b
   * Preventive Care and Screening Measure Pair: a. Tobacco Use Assessment, b. Tobacco Cessation
   */
  static function getNQF0028a($from, $to, $userId) {
    return CmsReport_NQF0028a::from($from, $to, $userId);
  }
  static function getNQF0028b($from, $to, $userId) {
    return CmsReport_NQF0028b::from($from, $to, $userId);
  }
  /**
   * NQF 0024
   * Weight Assessment and Counseling for Children and Adolescents
   */
  static function getNQF0024($from, $to, $userId) {
    return array(
      CmsReport_NQF0024_P1N1::from($from, $to, $userId),
      CmsReport_NQF0024_P1N2::from($from, $to, $userId),
      CmsReport_NQF0024_P1N3::from($from, $to, $userId),
      CmsReport_NQF0024_P2N1::from($from, $to, $userId),
      CmsReport_NQF0024_P2N2::from($from, $to, $userId),
      CmsReport_NQF0024_P2N3::from($from, $to, $userId),
      CmsReport_NQF0024_P3N1::from($from, $to, $userId),
      CmsReport_NQF0024_P3N2::from($from, $to, $userId),
      CmsReport_NQF0024_P3N3::from($from, $to, $userId));
  }
}
/**
 * CmsReport
...
abstract class CmsReport extends Rec {
  ...
  public function calculate($ugid, $from, $to, $userId) {
    logit_r("calculate($ugid,$from,$to,$userId)");
    $pops = $this->fetchPop($ugid, $from, $to, $userId);
    $nums = $this->fetchNum($ugid, $from, $to, $userId);
    $excs = $this->fetchExc($ugid, $from, $to, $userId);
  ...
  //
  static function from($from, $to, $userId) {
    $me = new static();
    $me->from = $from;
    $me->to = $to;
    $me->UserGroup = UserGroups::getMine();
    $me->User = User_Cms::fetchMine();
    $me->calculate($me->UserGroup->userGroupId, $from, $to, $userId);
    return $me; 
  }
  protected function fetchAllBy($c) {
    $this->_criteria[] = $c;
    return SqlRec::fetchAllBy($c, null, 2000);
  }
  protected function fetchMerge() {
    ...
    foreach ($crits as $crit) 
      $recs = array_merge($recs, SqlRec::fetchAllBy($crit, null, 2000));
    return $recs;
  }
  static function monthsPast($t1 = 1, $t2 = 2, $months = 6) {
    $t1 = $t1 * 2 + 1;
    $t2 = $t2 * 2 + 1;
    return CriteriaValue::sql("PERIOD_DIFF(EXTRACT(YEAR_MONTH FROM from_unixtime(t$t1.d)),EXTRACT(YEAR_MONTH FROM from_unixtime(t$t2.d))) BETWEEN 0 AND $months");
  }
}
/**
 * SQL Records
...
class Vital_Cms extends SqlRec implements ReadOnly {
  ...
  static function asActive() {
    $c = new static();
    $c->active = true;
    $c->Hd_date = Hdata_VitalsDate::join();
    return $c;
  }
}
...
class Proc_Cms extends SqlRec implements ReadOnly {
  ...
  static function from($ipc, $from = null, $to = null, $userId = null) {
    $c = new static();
    $c->ipc = $ipc;
    if ($from && $to)
      $c->setDates($from, $to);
...
class User_Cms extends SqlRec implements ReadOnly {
  ...
  static function fetchMine() {
    global $login;
    return User_Cms::fetch($login->userId);
  }
}
/**
 * CMS Scorecard

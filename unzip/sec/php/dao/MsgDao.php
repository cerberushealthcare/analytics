...
SELECT thread_id
FROM msg_inbox
WHERE recipient=$login->userId AND is_read=0
GROUP BY thread_id;
eos;
...
FROM msg_posts p
LEFT OUTER JOIN msg_inbox i ON (i.post_id=p.post_id AND i.recipient=$login->userId)
...
eos;
  ...
  public static function createThread($subject, $sendTos, $body, $data, $cid = null, $type = MsgDao::TYPE_GENERAL, $priority = MsgDao::PRIORITY_NORMAL) {
    ...
    $myName = quote($login->User->name, true);
    $cid = quote($cid);
...
VALUES(NULL,$login->userGroupId,$cid,$login->userId,$myName,NULL,NULL,NULL,$type,$status,$priority,$subject);
eos;
    $mtid = insert($sql);
...
VALUES(NULL,$mtid,$action,NULL,$login->userId,$myName,$body,$sendToNames,$data);
eos;
    $mpid = insert($sql);
...
UPDATE msg_inbox
SET is_read=2
WHERE recipient=$login->userId AND thread_id=$mtid AND is_read=1;
eos;
    query($sql);
  }
  /*
...
FROM users
WHERE user_group_id=$login->userGroupId AND active=1
ORDER BY name;
eos;
  ...
  public static function getMyRecipientGroups($asMap = false) {
    ...
    foreach ($recips as $id => &$r) {
      if ($id != $login->userId) {
        switch ($r['type']) {
          case User0::USER_TYPE_DOCTOR:
            $groups['d'][$id] = $r;
            break;
          case User0::USER_TYPE_OFFICE_EDITOR:
          case User0::USER_TYPE_OFFICE_READER:
            $groups['o'][$id] = $r;
            break;
          case User0::USER_TYPE_RECIPIENT_EDITOR:
          case User0::USER_TYPE_RECIPIENT_READER:
            $groups['r'][$id] = $r;
            break;
        }
      }
...
eos;
   ...
   */
  public static function buildSendToNames($sendTos) {
    global $login;
    if ($sendTos == null) {
      return null;
    }

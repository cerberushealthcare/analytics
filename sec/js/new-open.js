...
function getEditHeaderPopInfoCallback(pop) {
  ...
  Pop.show("pop-eh", "pop-eh-sendtos");
}
function saveHeader() {
  Pop.Working.show();
...
function getSession(sid) {
  Pop.Working.show("Retrieving document");
...
function buildResultsTable(clients) {
  ...
  if (clients != null) {
    for (var i = 0; i < clients.length; i++) {
      var c = clients[i];
      if (c.clientId > 0) {
        var href = (nnNextPop != 2) ? "javascript:selectClient(" + i + ")" : "javascript:showClient(" + c.clientId + ")";
...
function getPresetsCallback(presets) {
  ...
  if (presets == null || presets.length == 0) {
    Pop.Msg.showImportant("No custom templates were found.");
    return;
  }
...
function openTemplate(id) {
  Pop.Working.show("Getting template");
...
function ccsReset() {
  Pop.Confirm.showYesNoCancel("This will reset to the default settings. Are you sure?", ccsResetConfirmed, true);
}
...
function pe1Edit() {
  ...
  showHideIf(po.c.clientId, "pe1-cmd-update", "pe1-cmd-add");
  Pop.show("pop-pe1", "pe1-pid");
}
function pe1Save() {
  ...
  Ajax.Facesheet.Patients.save(po.c, 
    function(c) {
      Pop.close();
      if (nniEdit) {
        nniEdit = false;
      ...
      if (po.c.clientId) {
        ...
        window.location.href = "face.php?id=" + c.clientId + "&pe=1";
      }
    });
}
function pe2Edit(i) {
...
function pe2EditInsurance(icard, seq) {
  if (icard == null) {
    icard = {clientId:po.c.clientId, seq:seq};
  }
  po.icard = icard;
  Pop.setCaption("pop-icard-cap-text", po.c.name + " - Insurance Info");
...
function pe2EditCustom() {
  Pop.setCaption("pop-custom-cap-text", po.c.name + " - Custom Fields");
...
function pe2SaveAddress(a) {
  pe2PostSave("action=saveClientAddress&id=" + po.c.clientId + "&obj=" + jsonUrl(a));
}
function pcuSave() {
  po.c.cdata1 = value("pcu-custom1");
  po.c.cdata2 = value("pcu-custom2");
  po.c.cdata3 = value("pcu-custom3");
  pe2PostSave("action=saveClient&id=" + po.c.clientId + "&obj=" + jsonUrl(po.c));
}
function pofSave() {
...
function picSave() {
  ...
  pe2PostSave("action=saveClientICard&id=" + po.c.clientId + "&obj=" + jsonUrl(c));  
}

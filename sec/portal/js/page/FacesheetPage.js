...
FacesheetPage = PortalPage.extend(function(self) {
  return {
    onbodyload:function() {
      self.facesheet = FacesheetTile.create(self.content)
        .bubble('onselectmed', self.med_onselect)
        .bubble('onselectproc', self.proc_onselect)
        .bubble('onreturn', self.facesheet_onreturn);
      self.detail = DetailTile.create(self.content).hide()
        .bubble('onreturn', self.detail_onreturn);
    },
    ...
    return Html.Tile.create(container, 'DetailTile').extend(function(self) {
      return {
        ...
        init:function() {
          ...
          self.detail = Html.Div.create('Detail').into(self).aug({
            lbl:function(label, value) {
              this.append(Html.Div.create().add(Html.Label.create(null, label)).add(Html.Div.create().html(value)));
            }
          })
        ...
        loadMed:function(rec) {
          ...
          self.detail.clean();
          self.detail.lbl('Directions:', rec._friendlySig);
          self.detail.add(Html.Div.create('mt10'));
          //self.detail.add(Html.AnchorAction.asNote('View information sheet'));
          self.detail.add(Html.AnchorAction.asMsg('Request refill', function() {
            PortalPage.go('messaging.php', {'a':'refill','m':rec.name})}))
        },
        loadProc:function(rec) {
          ...
          if (rec._results) 
            self.detail.lbl('Results:', rec._results.join('<br>'));
          //self.detail.add(Html.Div.create('mt10'));
    ...
    return Html.Tile.create(container).extend(function(self) {
      return {
        ...
        draw:function() {
          ...
          My.DataTile.asMeds(into).load(self.fs)
            .bubble('onselect', self.med_onselect);
          My.DataTile.asProcs(into).load(self.fs)
            .bubble('onselect', self.proc_onselect);;
    ...
    create:function(container) {
      return Html.Tile.create(container).extend(function(self) {
        return {
          ...
          loadNone:function(li) {
            li.add(Html.Span.create('fs', 'None Recorded'));
          },
    ...
    asMeds:function(container) {
      return this.create(container).extend(function(self) {
        return {
          ...
          loadItem:function(li, rec) {
            if (rec.text == 'None Active')
              li.add(Html.Span.create('fs', 'None Active'));
            else

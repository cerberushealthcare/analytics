...
class TemplateReaderDao {
  ...
  public static function getPars($sectionId) {
    ...
    $res = query("SELECT par_id, section_id, uid, major, sort_order, `desc`, no_break, inject_only, date_effective, in_data_type, in_data_table, in_data_cond, current FROM template_pars WHERE section_id=" . $sectionId . " ORDER BY `desc`, date_effective DESC");
    $dtos = array();
    while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
      $dto = TemplateReaderDao::buildPar($row);
 ...
 // Get specific paragraph
  public static function getPar($parId, $withChildren = false, $withGrandchildren = false) {
    ...
    $par = TemplateReaderDao::buildPar(fetch("SELECT par_id, section_id, uid, major, sort_order, `desc`, no_break, inject_only, date_effective, in_data_type, in_data_table, in_data_cond FROM template_pars WHERE par_id=" . $parId));
    if ($par != null) {
  ...
  public static function buildPar($row) {
    if (! $row) return null;
    return new Par($row["par_id"], $row["section_id"], $row["uid"], $row["major"], $row["sort_order"], $row["desc"], $row["no_break"], $row["inject_only"], $row["date_effective"], $row["in_data_table"], $row["in_data_type"], $row["in_data_cond"]);
  }

...
Ajax.Ccd = {
   ...
   * @arg int procId
   */
  print:function(filename, cid) {
    Html.ServerForm.submit(this._SVR, 'print', {'filename':filename, 'cid':cid});
  }
}
Ajax.Ipc = {
  _SVR:'Ipc',
   ...
   * @arg fn(Ipc[]) callback
   */
  allHmIpc:function(worker, callback) {
    Ajax.getr(Ajax.Ipc._SVR, 'allHmIpc', null, null, worker, callback);
  },
  /*
   * @arg fn(Client_Rep[]) callback
   */
  allDueNow:function(worker, ipc, callback) {
    Ajax.getr(Ajax.Ipc._SVR, 'allDueNow', ipc, null, worker, callback); 
  },
  /*
   * @arg Client_Rep[] recs
   */
  downloadDueNow:function(worker, ipc) {

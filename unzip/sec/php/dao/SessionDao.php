...
class SessionDao {
  ...
  public static function addSession($userGroupId, $templateId, $clientId, $schedId, $dos, $templatePresetId = null, $sendTo = null, $dupeSessionId = null, $overrideFsActions = false) {
    ...
    if ($templatePresetId == null && $dupeSessionId == null) {
      ...
      if ($templatePresetId != null) {
        ...
        $data = decr($row["data"]);
        $noteDate = decr($row["note_date"]);
        if ($noteDate == null) $noteDate = decr($row["date_created"]);
      }
    }
    $title = LoginDao::authenticateReadTemplateId($templateId);
    if ($login->User->isDoctor()) {
      $assignedTo = $login->userId; 
    } else {
      $assignedTo = $sendTo;
    }
    $suppressFsActions = ! $overrideFsActions;
    logit("overridefs=" . $overrideFsActions . ", suppressfs=" . $suppressFsActions);
    // $suppressFsActions = ($login->userGroupId != 1094);
    $actions = DataDao::arrayifySessionActions($data, $noteDate, $templateId, $suppressFsActions);
    DataDao::appendInData($actions, $noteDate, $templateId, $clientId, $dos, $suppressFsActions);
      //$actions2 = DataDao::inActionsForGetPar(2631, null, 3484);
      //logit_r($actions2, 'actions2');
      //$actions = array_merge($actions, $actions2);
    $data = DataDao::actionsAsString($actions);
    $sql = "INSERT INTO sessions VALUES(NULL";
    $sql .= ", " . $userGroupId;
    $sql .= ", " . $clientId;
    $sql .= ", " . $templateId;
    $sql .= ", " . encr(nowNoQuotes());  // date_created
    $sql .= ", NULL";  // date_updated
    $sql .= ", " . encr(dateToString($dos)); // date_service
    $sql .= ", " . toBoolInt(false);  // closed
    $sql .= ", NULL";  // closed by
    $sql .= ", " . encr(nowNoQuotes());  // date closed
    $sql .= ", 0";  // billed
    $sql .= ", ";
    $sql .= ($schedId != null) ? $schedId : "NULL";
    $sql .= ", " . encr($data, true);
    $sql .= ", " . $login->userId;  // created by
    $sql .= ", NULL";  // updated by
    $sql .= ", " . quote($sendTo); 
    ...
    $sql .= ", 0";  // standard
    $sql .= ", " . encr($noteDate);
    $sql .= ", NULL, NULL, NULL";  // stub
    $sql .= ")";

    $id = insert($sql);
    //AuditDao::log($clientId, AuditDao::ENTITY_SESSION, $id, AuditDao::ACTION_CREATE, null, $title);
    $sess = SessionNote::fetch($id);
    if ($dupeSessionId) {
      $title .= " (from $dupeSessionId)";
      Auditing::log(AuditRec::ACTION_COPY, $clientId, 'Session', $id, $title, null, AuditImage::from($sess));
    } else {
      Auditing::log(AuditRec::ACTION_CREATE, $clientId, 'Session', $id, $title, null, AuditImage::from($sess));
    }
    UsageDao::createUsageDetail($id, SchedDao::USAGE_SESSION, $clientId, "");
    Hdata_SessionDos::create($userGroupId, $id, $dos)->save();
    return $id;
  }
  /**
  ...
  public static function unsign($sid) {
    global $login;
    if ($login->admin)
      return query("UPDATE sessions SET closed=0 WHERE session_id=$sid");
  }
  ...
  public static function updateSessionEnvelope($se) {
    ...
    $sql = "UPDATE sessions SET ";
    $sql .= "date_service=" . encr(dateToString($se->dateService));  
    $sql .= ", send_to=" . quote($se->sendTo); 
    $sql .= ", date_updated=NULL, updated_by=" . $myUserId;
    $sql .= " WHERE session_id=" . $se->id;
    $q = query($sql);
    Hdata_SessionDos::create($login->userGroupId, $se->id, $se->dateService)->save();    
    return $q;
  }
  ...
  public static function deleteSession($sessionId) {
    ...
    Auditing::log(AuditRec::ACTION_DELETE, $clientId, 'Session', $sessionId);
  }
  ...
  public static function getMyPresetsAsRows($withActions = false) {
    ...
    $sql .= " FROM template_presets WHERE user_group_id=" . $login->userGroupId . " ORDER BY name";
    return query($sql);
  }
  ...
  public static function getMyTemplatePresetsAsRows() {
    ...
    $sql .= " FROM template_presets p, templates t WHERE p.template_id=t.template_id AND p.user_group_id=" . $login->userGroupId . " ORDER BY p.name";
    return query($sql);
  }
  ...
  public static function getJTemplatePresets($where = " ORDER BY date_updated DESC") {
    ...
    $sql .= "WHERE tp.template_id=t.template_id AND tp.user_group_id=" . $login->userGroupId;
    $sql .= " " . $where;
    $res = query($sql);
    $dtos = array();
    while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
      $dtos[] = new JTemplatePreset($row["template_preset_id"], $row["user_group_id"], $row["preset_name"], $row["template_id"], decr($row["date_created"]), $row["date_updated"], $row["template_name"], null, $row["created_by_name"], $row["updated_by_name"]);
    }
    return $dtos;
  }
  ...
  public static function getJTemplatePreset($id) {
    ...
    $sql .= " LEFT OUTER JOIN users u2 ON (u2.user_id=tp.created_by)";
    $sql .= " LEFT OUTER JOIN users u3 ON (u3.user_id=tp.updated_by)";
    $sql .= "WHERE tp.template_id=t.template_id AND tp.user_group_id=" . $login->userGroupId . " AND tp.template_preset_id=" . $id;
    $row = fetch($sql);
    if ($row) {
  ...
  public static function getPresetIdByName($name) {
    global $login;
    $row = fetch("SELECT template_preset_id FROM template_presets WHERE user_group_id=" . $login->userGroupId . " AND name=" . quote($name));
  ...
  public static function getIcdCodes($code) {
    ...
    $sql = "SELECT icd_code, icd_desc, synonyms, includes, excludes, notes FROM icd_codes5 WHERE icd3='" . $icd3 . "' ORDER BY icd_code";
    $res = query($sql);
    $dtos = array();
    while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
  ...
  public static function searchIcdCodes($text) {
    ...
    $words = explode("|", $text);
    $words = count($words);
    $sql = "SELECT DISTINCT icd3 FROM icd_codes5 WHERE icd_code <= '999' AND icd_code RLIKE '" . $text . "' OR icd_desc RLIKE '" . $text . "' OR synonyms RLIKE '" . $text . "'";
    $res = query($sql);
    $in = "";
    $icd3count = 0;
    $more = false;
    ...
    if ($in != "") {
      $sql = "SELECT icd3, icd_code, icd_desc, synonyms, includes, excludes, notes FROM icd_codes5 WHERE icd3 IN (" . $in . ") ORDER BY icd_code";
      $res = query($sql);
      $pattern = "/" . $text . "/i";
  ...
  public static function isLocked($sessionId) {
    $closed = fetchField("SELECT closed FROM sessions WHERE session_id=$sessionId");
    return ($closed && $closed>'0');
  }
  
  // Update session's actions only
  public static function updateSessionActions($sessionId, $title, $actions, $html = null, $stub = null) {
    global $login;
    $clientId = LoginDao::authenticateSessionId($sessionId);
    $stubId = get($stub, 'id');
    $stubType = get($stub, 'type');
    $stubName = get($stub, 'name');
    $sql = "UPDATE sessions SET ";
    $sql .= "data=" . encr($actions, true);
    if ($login->User->isDoctor()) {
      $sql .= ", assigned_to=" . $login->userId;      
    }
    $sql .= ", stub_id=" . quote($stubId) . ", stub_type=" . quote($stubType) . ", stub_name=" . quote($stubName);
    $sql .= ", date_updated=NULL, updated_by=" . $login->userId;
    if ($html)
      $sql .= ", html=" . encr($html, true);
    $sql .= " WHERE session_id=" . $sessionId;
    query($sql);
  ...
  public function updateVisitSum($o) {
    $iols = $o->iols;
    $instructs = $o->instructs;
    logit_r($o, 'updateVisitSum');
    $diagnoses = static::getDiagnosesFromOut($o->out);
    $vitals = static::getVitalsFromOut($o->out);
    $meds = static::getMedsFromOut($o->out);
    logit_r($meds, 'meds');
    logit_r($diagnoses, 'diagnoses');
    //logit_r($vitals, '$vitals');
    $sql = "INSERT INTO client_visitsums (client_id, final_id, dos, session_id, finalized_by, final_head, final_body, diagnoses, iols, instructs, vitals, meds) VALUES("
    	. $o->cid . ','
    	. '0,' 
    	. encr($o->dos) . ','
    	. $o->sid . ','
    	...
    	. 'null,'  // final_head
    	. 'null,'  // final_body
    	. encr(jsonencode($diagnoses), true) . ','
    	. encr($iols, true) . ','
    	. encr(jsonencode($instructs), true) . ','
    	. encr(jsonencode($vitals), true) . ','
    	. encr(jsonencode($meds), true)
    	. ') ON DUPLICATE KEY UPDATE dos=VALUES(dos), session_id=VALUES(session_id), diagnoses=VALUES(diagnoses), iols=VALUES(iols), instructs=VALUES(instructs), vitals=VALUES(vitals), meds=VALUES(meds)';
    query($sql); 	
    //logit_r($sql, 'for updatevisitsum');
  }
  private static function getDiagnosesFromOut($out) {
    $a = array();
    $diags = getr($out, 'diagnoses.records');
    if ($diags) {
  ...
  private static function getMedsFromOut($out) {
    $a = array();
    $meds = getr($out, 'meds.records');
    if ($meds) {
      foreach ($meds as $key => $med) {
        $keys = explode('|', $key);
        $index = end($keys);
        $action = static::getMedAction(prev($keys));
        if ($action) {
          $o = $med->fields;
          $o->action = $action;
          $a[] = $o;
  ...
  private static function getMedAction($key) {
    $parts = explode('.', $key);
    if ($parts[0] == 'med mgr' || $parts[0] == 'plan')
      return end($parts);
  }
  private static function getVitalsFromOut($out) {
    $vits = getr($out, 'vitals.records');
    if ($vits) {
      foreach ($vits as $key => $vit) {
        $keys = explode('|', $key);
        $o = $vit->fields;
        $o->date = end($keys);
        return $o;
  ...
  public static function saveSessionHtml($sessionId, $html) {
    ...
    LoginDao::authenticateSessionId($sessionId);
    $sql = "UPDATE sessions SET html=" . encr($html, true) . " WHERE session_id=" . $sessionId;
    return query($sql);
  }
  
  public static function addendum($sessionId, $html) {
    LoginDao::authenticateSessionId($sessionId);
    $row = fetch("SELECT html FROM sessions where session_id=$sessionId");
    if ($row) {
      $ohtml = decr($row['html']);
      $add = "<p></p><div id=addendum>" . SessionDao::buildSigTable(SessionDao::buildSignature("Addendum")) . "<div><p>" . $html . "<br><br></p></div></div>";
      $html = $ohtml . $add;
      $sql = "UPDATE sessions SET html=" . encr($html, true) . " WHERE session_id=" . $sessionId;
      return query($sql);
    }
    Auditing::log(AuditRec::ACTION_SIGN, $clientId, 'Session', $sessionId, $title, null, $after);
  }
  ...
  public static function saveSession($sessionId, $actions, $title, $dos, $standard, $close, $nosign, $html) {
    ...
    */
    $sql = "UPDATE sessions SET ";
    $sql .= "data=" . encr($actions, true);
    $sql .= ", date_service=" . encr(dateToString($dos));
    $sql .= ", standard=" . toBoolInt($standard);
    $sql .= ", title=" . encr($title, true);
    if ($close) {
      $sql .= ", html=" . encr($html, true);
      //$sql .= ", closed='3', date_closed=" . encr(nowNoQuotes()) . ", closed_by=" . $login->userId;
    ...
    if ($close) {
      require_once 'php/c/sessions/Sessions.php';
      Sessions::sign($sessionId);
      /*
      $after = new AuditImageSession($actions);
      Auditing::log(AuditRec::ACTION_SIGN, $clientId, 'Session', $sessionId, $title, null, $after);
      */
    }

...
class JDataAllergy {
  ...
  public $source;
  //
  const SQL_FIELDS = 'data_allergy_id, user_group_id, client_id, session_id, date, `index`, agent, reactions, active, date_updated, source';
  ...
  public static function save($dto, $escape = false, $audit = true) {
    if ($dto->id != null) {
      ...
      $sql .=" WHERE data_allergy_id=" . $dto->id;
  ...
  // Return collection of active non-session data records
  ...
  /**
   * Fetch active allergies (New Crop source)
   * @param int $clientId
   * $return array('agent'=>JDataAllergy,..)
   */
  public static function getActiveNewCropAllergies($clientId) {
    $sql = "SELECT " . JDataAllergy::SQL_FIELDS . " FROM data_allergies WHERE client_id=" . $clientId . " AND session_id IS NULL AND source=1 AND active=1";
    return JDataAllergy::fromRows(query($sql), "agent");
  }
  ...
  public static function getAllergy($id) {
    $sql = "SELECT " . JDataAllergy::SQL_FIELDS . " FROM data_allergies WHERE data_allergy_id=" . $id;
  ...
  public static function getClientAllergyByAgent($clientId, $agent) {
    $row = fetch("SELECT data_allergy_id FROM data_allergies WHERE session_id IS NULL and client_id=" . $clientId . " AND agent=" . quote($agent, true) . " ORDER BY data_allergy_id DESC");
    if ($row) {
      return $row["data_allergy_id"];
    }
  }

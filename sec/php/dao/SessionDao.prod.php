...
class SessionDao {
  ...
  public function updateVisitSum($o) {
    ...
    $sql = "INSERT INTO client_visitsums (client_id, final_id, dos, session_id, finalized_by, final_head, final_body, diagnoses, iols, instructs, vitals, meds) VALUES("
    	...
    	. 'null,'  // final_head
    	. 'null,'  // final_body
    	. quote(jsonencode($diagnoses), true) . ','
    	. quote($iols, true) . ','
    	. quote(jsonencode($instructs), true) . ','
    	. quote(jsonencode($vitals), true) . ','
    	. quote(jsonencode($meds), true)
  ...
  public static function saveSession($sessionId, $actions, $title, $dos, $standard, $close, $nosign, $html) {
    ...
    if ($close) {
      ...
      $sql .= ", closed='3', date_closed=NULL, closed_by=" . $login->userId;

...
class Audit_Rep extends AuditRec implements ReportRec_CsvJoinable {
  ...
  public function toJsonObject(&$o) {
    ...
    $o->after = $this->decorateSnapshot($this->after, $this->hrec);
    $o->before = $this->decorateSnapshot($this->before);
    if ($this->hrec) {
      $o->_hash = $this->hash();
      if ($this->hrec != $o->_hash) {
        $o->_altered = 1;
      }
    }
  }
  private function decorateSnapshot($rec, $hrec = null) {
  ...
  public function formatNameLabel() {
    $s = $this->recName;
    if ($this->label)
      $s .= ': ' . $this->label;
    return $s;
...
class Med_Rep extends Med implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatTimestampOptional($this->date) . ': ' . $this->name;
  }
  public function getTableName() {
    return 'Medications'; 
  }
  ...
  static function asCriteria($ugid) {
    ...
    //$c->date = CriteriaValue::isNotNull();
    return $c;
  }
}
...
class MedHist_Rep extends SessionMedNc implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatTimestampOptional($this->date) . ': ' . $this->getQuidText() . ': ' . $this->name;
  }
  public function getTableName() {
    return 'Med History';
  }
...
class Diagnosis_Rep extends FaceDiagnosis implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatTimestampOptional($this->date) . ': ' . $this->formatName();
  }
  public function getTableName() {
    return 'Diagnoses';
  }
...
class RepCrit_Session extends RepCritRec {
  ...
  public function getRecSort() {
    return new RecSort('dateService');
  }
  protected function assignSqlCriteriaValue(&$criteria, $fid, $value) {
    global $login;
    $cv = RepCritValue::asSqlCriteriaValue($criteria, $fid, $value);
...
class Session_Rep extends SessionNote implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatTimestampOptional($this->dateService) . ': ' . $this->formatTitle();
  }
  public function getTableName() {
    return 'Sessions';
  }
...
class Allergy_Rep extends Allergy implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatTimestampOptional($this->date) . ': ' . $this->agent;
  }
  public function getTableName() {
    return 'Allergies';
  }
...
class ProcOfficeVisit_Rep extends Proc implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatTimestampOptional($this->date) . ': ' . (($this->User) ? $this->User->name : '');
  }
  public function getTableName() {
    return 'Encounter';
  }
...
class RepCrit_AdminIpc extends RepCritRec {
  //
  public $ipc;
  public $date;
  public $userId;
...
class ProcAdminIpc_Rep extends Proc implements ReportRec_CsvJoinable {
  //
  public function toJsonObject(&$o) {
    $o->_label = $this->formatLabel();
    $o->_table = $this->getTableName();
  ...
  public function formatLabel() {
    return formatTimestampOptional($this->date) . ': ' . (($this->Ipc) ? $this->Ipc->name : '');
  }
  public function getTableName() {
    return 'MU Event';
  }
...
class Proc_Rep extends Proc implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatApproxDateTime($this->date) . ': ' . (($this->Ipc) ? $this->Ipc->name : '');
  }
  public function getTableName() {
    return 'Procedures';
  }
...
class ProcResult_Rep extends ProcResult implements ReportRec_CsvJoinable {
  ...
  //}
  public function formatLabel() {
    return formatApproxDateTime($this->Proc->date) . ': ' . self::summarizeResult(null, $this);
  }
  public function getTableName() {
    return 'Results';
  }
...
class Immun_Rep extends Immun implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatApproxDateTime($this->dateGiven) . ': ' . $this->name;
  }
  public function getTableName() {
...
class Vital_Rep extends Vital implements ReportRec_CsvJoinable {
  ...
  public function formatLabel() {
    return formatTimestampOptional($this->date) . ': ' . join(', ', $this->getAllValues());
  }
  //
  static function asCriteria($ugid) {
    $c = new self();

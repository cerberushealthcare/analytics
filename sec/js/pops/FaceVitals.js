...
FaceVitals = {
  ...
  pop:function(fs, id, zoom, callback) {
    ...
    var self = this;
    Includer.get(Includer.HTML_FACE_VITALS, function() {
      if (fs.client) {
  ...
  fpPrint:function() {
    if (me.Role.Patient.vitals)
    Pdf_Vitals.from(this.fs, _$('fsp-vith-div')).download();
  ...
  _load:function(tp) {
    ...
    //_$('vit-cmd-chart').showIf(me.Role.Patient.vitals);
    //_$('vit-cmd-add').showIf(me.Role.Patient.vitals);
    if (fs.vitals) {
      fs.vitalsById = {};

...
FaceHm = {
  ...
  pop:function(fs, procId, zoom, callback) {
    ...
    //Includer.get([Includer.HTML_FACE_HM, Includer.AP_TEMPLATE_POPS], function() {
      if (fs.client) {
        Pop.setCaption("fsp-hm-cap-text", fs.client.name + " - Health Maintenance");
        Pop.setCaption("pop-hme-cap-text", fs.client.name + " - Health Maintenance Entry");
      }
      self._load();
      ...
      if (zoom) {
        Pop.zoom(FaceHm._POP, null, function(){self._pop()});
      } else {
        Pop.show(FaceHm._POP);
        self._pop();
      }
    //});
  },
...
FaceHmIntEntry = {
  ...
  pop:function(callback) {
    ...
    Pop.show("fsp-hmcint", "hmcint-every", true);
  },
  fpOk:function(pcid) {
    var fs = this.fs;
...
FaceHmCustomProc = {
  ...
  fpaSave:function() {
    ...
    if (isBlank(rec.name) || isBlank(rec.id)) {
      Pop.Msg.showCritical('Name and CPT are required fields.');
      return;
    }
    if (this.fs.hmProcs[rec.id]) {
      Pop.Msg.showCritical('This CPT code is already in use.');
      return;
    }
    Pop.close();
    this._buildRow(rec);

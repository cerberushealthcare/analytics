...
var ErxStatusPage = {
  ...
  reset:function() {
    ...
    Ajax.Erx.getStatusDetails(
      function(o) {
        var recs = o.recs;
        self._load(recs, page._getFilter(o.ct.lpName));
  ...
  _load:function(recs, filter) {
    ...
    for (var i = 0, j = recs.length; i < j; i++) {
      ...
      var doc = rec.UserStub_Doctor && rec.UserStub_Doctor.name;
      var name = (rec.staff) ? doc + ' / ' + rec.staff : doc;
      if (status != last) {
        t.createTr(true);
  ...
  _getClientAnchor:function(rec) {
    ...
    if (client) {
      ...
      if (rec.clientId == 'NOTFOUND0') {
        ...
        a = Html.Span.create(null, rec.patientName);  // a.href = 'javascript:page.pNewCrop(null, "' + dest + '")';
      }  
    }
    return a;
  },

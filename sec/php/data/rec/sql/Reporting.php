...
require_once 'php/data/rec/sql/Reporting_Metrics.php';
/**
 * Reporting DAO
...
class Reporting {
  ...
  static function /*ReportCriteria*/generateForDownload(/*obj*/$rc, /*bool*/$numerator, /*bool*/$noncomps) {
    global $login;
    ...
    if ($noncomps) {
      return static::generateNoncomps($criteria);
    } else {
...
class ReportCriteria extends Rec implements SerializeNulls {
  ...
  public function isHistable() {
    return $this->RecDenom && ($this->type == self::TYPE_PATIENT || $this->type == self::TYPE_MU); 
  }
  ...
  protected static function fromUi($ugid, $o) {
    $rec = new static($ugid, $o->reportId, $o->name, $o->type, get($o, 'comment'), get($o, 'countBy'));
...
require_once 'php/data/rec/sql/Reporting_Recs.php';
p_i('/Reporting');

...
class UserAdmin extends UserRec implements ReadOnly {
  ...
  public function getHtmlTds($vert = false) {
    ...
    $rec->name = UserAdministration::asAnchor('userByUid', $rec->uid, $rec->name);
    $rec->uid = UserAdministration::asAnchor('loginsByUid', $rec->uid);
    $rec->dateCreated = ($vert) ? formatInformalTime($rec->dateCreated) : formatInformalDate($rec->dateCreated);
    $rec->active = ($rec->isActive()) ? 'Yes' : '';
    if ($rec->UserGroup)
    ...
    if ($rec->BillCode) 
      $rec->BillCode = UserAdministration::asAnchorBillCode($rec->BillCode); 
    ...
    if ($rec->isSupport()) 
      ...
      $rec->subscription = UserAdministration::asAnchorSubscription($rec->subscription);
    unset($rec->regId);
  ...
  public static function fetchAllActiveSubscribers($subscription = null) {
    if ($subscription == User::SUBSCRIPTION_TRIAL)
...
class Login extends SqlRec implements ReadOnly {
  ...
  public function getHtmlTds() {
    $this->uid = UserAdministration::asAnchor('loginsByUid', $this->uid);
    $this->logdate = formatInformalTimeDay($this->logdate);
    $this->ipAddress = $this->asAnchor();
    if ($this->User) 
      $this->User = Useradministration::asAnchorUser($this->User); 
    return $this->getValues();
  }
  public function asAnchor() {
    return UserAdministration::asAnchor('loginsByIp', $this->ipAddress);
  }
  //
...
class UserGroupAdmin extends SqlRec implements ReadOnly {
  ...
  public function asAnchor() {
    return UserAdministration::asAnchor('usersByUgid', $this->userGroupId, $this->name);    
  }
  //
  /**
   * @param int $ugid
...
class CreatedCount extends Rec {
  ...
  public function getHtmlTds() {
    ...
    $this->count = UserAdministration::asAnchor('usersByCreateDate', $this->date, $text);
...
class BillInfoStub extends SqlRec implements ReadOnly, CompositePk {
  //
  public $userId;
...
class BillCodeStub extends SqlRec implements ReadOnly {
  //
  public $billCode;

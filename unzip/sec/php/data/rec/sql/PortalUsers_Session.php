...
class PortalUsers_Session {
  ...
  static function respond($cas) {
    ...
    if (! $rec->answer($cas[0], $cas[1], $cas[2], $cas[3])) 
 ...
 */
class PortalSession extends LoginSession {
  //
  public $portalUserId;
  public $userGroupId;
  public $erx;
...
class PortalLogin extends PortalLoginRec implements NoAuthenticate {
  //
  public $logId;
  public $logDate;
  public $logIp;
 ...
 */
class PortalUser extends PortalUserRec implements NoAuthenticate {
  //
  public $portalUserId;
  public $userGroupId;
  public $clientId;
  public $uid;
  public $pw;  // encrypted client-set password
  public $active;
  public $status;
  public $ssn4;
  public $createdBy;
  ...
  public $cq3;
  public $ca3;
  public $pwpt;  // auto-generated initial password
  public $tosAccept;
  public $subscription;
  ...
  public function needsPortalTos() {
    return $this->tosAccept < '2012-06-24';
  }
  public function acceptTerms() {
    $this->tosAccept = nowNoQuotes();
    $this->save();
  }
  /**
   * @return bool true on success, false if answer mismatch
   */
  public function answer($ssn4, $ca1, $ca2, $ca3) {
    if ($this->ssn4 == $ssn4 && $this->matchesAnswers($ca1, $ca2, $ca3)) {
  ...
  public function changePassword($plainOld, $plainNews) {
    if (! $this->matchesPassword($plainOld))
  ...
  public function establishPassword($plainNews) {
    ...
    Password::testStrength($plainNews[2]);
...
class User_Ps extends UserRec implements ReadOnly {
  ...
  static function asIdNameMap($recs) {
    $map = array();
    foreach ($recs as $rec) 
      $map[$rec->userId] = $rec->name;
    return $map;
  }
}
/**
...
class Password {
  ...
  public function matches($hashed) {
    ...
    logit_r($hash, 'hash');
    logit_r($hashed, 'hashed');
    return self::hash($this->plain, $hashed) == $hashed;
  }
  //
  static function testStrength($strength) {
    if ($strength < 1)
      throw new PasswordException('Password is not strong enough.');
  }
  //
...
//
class Proc_PortalAccess extends SqlRec implements NoAudit {
  static function record($ugid, $cid) {
    $me = new static();
    $me->userGroupId = $ugid;
    $me->clientId = $cid;
    $me->date = nowNoQuotes();

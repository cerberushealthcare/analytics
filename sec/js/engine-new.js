...
var changed;       // {qid:q}  questions to process in doOnChanges
var changed2;      // {qid:q}  questions to process in next iteration of doOnChanges
var dirty = false;
var now = new Date();
...
function resetUi() {
  tlog("resetUi", true);
  ignoreActionErrors = (perm.ugid == 24);
  if (session) {
    session.ro = perm.ro;
...
function startSessionActions() {  
  ...
  closeWorking();  // no new thread spawned, we're done
}
/*
 * Prepend actions.stack with any user-customized autopars
 */
function prependMapAutoActions() {
  for (var sid in map.sections) {
    var s = map.sections[sid];
    for (var pid in s.pars) {
      if (s.pars[pid].auto) {
        actions.stack.unshift("getPar(" + pid + ")");
      }
    }
  }  
}
...
function parseParInfos(pis) {
  ...
  if (actions.getting) {  // This is the initial collection of requested pars + conditionless injects... no special processing required, just get everything
    ...
    doOnChanges('parseParInfos actions.getting');
    finishActions();
    return;
  }
  ...
  waitingOnPar--;
  clog("waitingOnPar--=" + waitingOnPar);
  if (waitingOnPar == 0 && ! actions.loading && showingLoading) {
  ...
  if (actions.inActionsAdded) {
    actions.loading = true;
    autosave(true);
    doOnChanges('parseParInfos requested');
    finishActions();
  }
  applyCustomShows();
}
function parseParInfosFromCache(pid, injector) {
  ...
  if (pi.autoInj) {
    for (var i = 0; i < pi.autoInj.length; i++) {
      var inj = (isCloneRef(pi.autoInj[i])) ? pid + csuf : null;
...
function parseParInfo(pi, injectRef) {  // injectRef (puid) is passed when this par comes from a conditioned inject by the injectRef par
  ...
  if (pi.csuf) {  // cloneable
    ...
    setCloneIds(pi.par.qTemplates, pi.csuf);
  ...
  for (var i = 0, i2 = pi.questions.length; i < i2; i++) {
    ...
    if (q.osyncs && (pi.suid == "hpi" || pi.suid == "meds")) {
      q.sid = pi.sid;
      q.noSyncIn = true;
    }  
  ...
  if (pi.inActions && actions && ! showingLoading) {
...
function finishRedoActions() {
  ...
  doOnChanges('finishRedoActions');  // TEST, used to pass questions
  applyCustomShows();
  autosave(true);
  closeWorking();
}
function removePar(pid, ct, wasRemoved) {  // ct=0 for non-cloneable, instance index for cloneable, wasRemoved=true action already removed
...
function sendGetParRequest(pid, injectorPid) {
  cloga("sendGetParRequest", [pid, injectorPid]);
  waitingOnPar++;
  clog("waitingOnPar++=" + waitingOnPar);
  if (jCache[pid] != null) {
...
//    }
    ...
    h.push(" class=h>");
    if (s.title) {
      h.push("<p class=pTitle>" + s.title + "</p>");
    } else {
      h.push("<p class=h></p>");
    }
    h.push("</div>");
...
//  }
  ...
  if (s.h != "true" && par.uid.substr(0, 1) != "&") {
    s.className = VISIBLE;
  }
  p.id = pidify(par.id);
  p.uid = par.uid;
  ...
  if (par.qTemplates.length == 1) {
    var q = par.qTemplates[0];
    ...
    var fragging = false;
    var q;
    for (var i = 0, j = par.qTemplates.length, k = j - 1; i < j; i++) {
      q = par.qTemplates[i];
  ...
  for (var i = 0; i < par.qTemplates.length; i++) {
    var qt = par.qTemplates[i];
    ...
    if (qt.test != null) {  // restored
      var test = appendPidiArgs(qt.test, pi);  // restored
      q.tag.className = visibleIf(eval(test));  // restored
    }  // restored
  }
...
function docAddQuestion(qt, pid, last, pi) {
  ...
  var notext = (q.opts == null && qt.bt == null && qt.at == null); 
  q.lt = qt.lt;
  var clazz = VISIBLE;
  var h = [];
  if (notext) {
  ...
  if (q.type == Q_TYPE_DATA_HM) {
    h.push("<span class=noprt prt=n style='display:block'><a href='javascript:' onclick='showPopHm(" + qt.id + ")'>Select HM History</a></span>");
    ...
    if (q.char1 == "@") {  
      ...
      var attribs = [
          "rxFt=" + bool(pi.suid == "med mgr" || pi.suid == "plan" || pi.suid == "treatment"),  // has rx freetext
          "rx=" + bool(rx),  // has rx button
          "refillable=" + bool(pi.suid == "meds" && q.uid == "@addMed"),  // included in rx pop
          "medNameOnly=0"];
//        "medNameOnly=" + bool(q.uid == "@dcMed")];
      var fn = "showPopMed()";
      if (q.uid == "@rfMed" && ! rx) {
        fn = "showPopRx()";
        medRefillId = qidify(qt.id) + "o";
      }
      h.push(docAddClonePop(qt, clazz, fn, addText, last, par.id, attribs));
    } else if (q.char1 == "!") {  
    
      // Allergy pop
      allergyInsertedQid = qt.id;
      h.push(docAddClonePop(qt, clazz, "showPopAllergy()", "Add Allergy", last, par.id));
    } else {
    ...
    if (q.opts != null) {
      ...
      h.push("<a href='javascript:return false' id=" + qidify(qt.id + "o") + " class=df onclick=\"showPopQuestion('" + qt.id + "'); return false\">");
      h.push("___");
      h.push("</a>");
      if (q.mix != null) {
        h.push("</span>");  //1
        ...
        h.push("<a href='javascript:return false' id=" + qidify(qt.id + "s") + " class=df onclick=\"showPopQuestion('" + qt.id + "'); return false\">");
        h.push("___");
        h.push("</a>");
        if (qt.lt != 2) {
          h.push(" ");
        ...
        if (qt.btmu != null) {
          ...
          h.push("<a href='javascript:return false' id=" + qidify(qt.id + "u") + " class=df onclick=\"showPopQuestion('" + qt.id + "'); return false\">");
          h.push("___");
          h.push("</a>");
          if (qt.atmu != null) {
    ...
    //if (q.icd != null) {
    //  h.push("&nbsp;<a class=df id=" + qidify(qt.id + "icd") + " href='javascript:return false' onclick='showPopIcd()'>(" + q.icd + ")</a>"); 
    //}
    if (q.lt != 2 && ! q.frag) {  // bullets
      h.push(breakTypeBefore(qt.brk));
    }
    if (qt.brk == 0 || qt.brk == 4 || qt.brk == 5) {
...
function docAddClonePop(qt, clazz, addFn, addText, last, pid, attribs) {  // optional extra attribs[] for question span
  var attr = (attribs) ? attribs.join(" ") : "";
  var qtid = qidify(qt.id);
  var h = [];
  h.push("<span name=clonePop " + attr + " qid=" + qt.id + " id=" + qtid + " pid=" + pid + " class=" + clazz + "><span>");
  if (qt.bt != null) {
    h.push(filter(qt.bt) + " ");
  }
  h.push("</span><span id=" + qtid + "c name=clonekids></span><a href='javascript:return false' qid=" + qt.id + " id=" + qtid + "o kid=" + qtid + "c class=clone onclick=\"" + addFn + "; return false\">");
  h.push(addText); 
  h.push("</a> <span>");
  if (qt.at != null) {
    h.push("&nbsp;" + filter(qt.at));
...
//    if (q.icd != null && o.icd != null) {
//      var icda = $(hqid + "icd"); 
//      icda.innerText = "(" + o.icd + ")";
//      icda.className = (q.icd == o.icd) ? "df" : "";
//    }
    if (q.frag) {
...
function addQTest(q, test, pi) {
  ...
  for (var ref in refs) {
    var qTests = qTestsByQref[ref];
    if (qTests == null) {
      qTestsByQref[ref] = [qTest];
    } else {
      qTestsByQref[ref].push(qTest);
    }
  }
}
...
function isRunOnce(cond, action) {
  ...
  if (word == "inject") return true;           // injects only run once  
...
/*
 * Cache question as "changed" for text/action processing (by doOnChanges) 
 */
function onChange(qid) {
  //if (qid == 10718) f.fred();
  var q = questions[qid];
  if (changed2) {  // called while in a doOnChanges loop
    changed2[qid] = q;
  } else {
    changed[qid] = q;
  }
...
/*
 * Calls doOnChange for each changed question "changed" while accumulating cascaded changes into "changed2"
 * Loops recursively until all affected qids are onChanged
 */
function doOnChanges(source) {
  changed2 = {};
  var i = 0;
  for (var qid in changed) {
    //if (qid == 10718) f.fred();
    doOnChange(qid);
    i++;
  }
  cloga("doOnChanges", [source], i);
  if (! isEmptyMap(changed2)) {
    changed = changed2;
    doOnChanges('within');
  } else {
    changed = {};
 ...
 */
function doOnChange(qid) {
  var q = questions[qid];
  if (q.opts && ! q.cloning) formatQuestionAndFrags(q);
  if (q.osyncs) syncOptions(q);
  if (q.sync) syncQuestion(q);
  runQTests(q.ref);
  runQActions(q.id);
  checkInjPars(q.ref);
}
 ...
 */
 function runQTests(qref) {
  var qTests = qTestsByQref[qref];
  if (qTests == null) return;
  var t;
  var vis;
 ...
 */
function setParVisibility(p, test) {
...
function startActions(msg) {
  ...
  actions.loading = true;
  showingLoading = true;
  if (msg != null) showWorking(msg, false, true);
  startGetPars();
}
...
function fixGetPar(i) {
  ...
  return isGetPar(actions.stack[i]);
}
...
function finishActions(restart) {
  ...
  while (actions.loading) {
    try {
      ...
      if (actions.restart < 10) {
        actions.restart++;
        doOnChanges('on error');  // TEST used to pass questions
        setTimeout("finishActions(1)", 1000);
        return;
      } else {
...
function closeLoading() {
  ...
  if (waitingOnPar == 0) {
    ...
    doOnChanges('closeLoading');  // TEST used to pass questions
...
function applyCustomShows() {
  if (perm.yk) showHideTags(!! lu_console.showAdminTags);
  if (showFT) showHideFreeTags();
...
function setDefault(qref, oref, _pidi, _csuf) {
  ...
  if (ix >= q.mix && q.sel.length == 0) {
    var unsel = [];
    for (var i = q.mix; i < q.opts.length; i++) {
      if (i != ix) unsel.push(i);
    }
    q.unsel = unsel;
  }
  clog('onChange(' + q.id + ')');
...
function filter(t, noHtml) {  // noHtml = don't add HTML tags 
...
function freeTextAnchorHtml(id) {
  return "<a class=ftd id=\"" + id + "ft\" href='javascript:return false' onclick=\"showFreePop('" + id + "'); return false\"></a>&nbsp;" 
}
function copy() {
  //us3();
...
function parseDataField(outData, value, a, dtid) {  // a=anchor instance if cloning pop, dtid only req for pk fields
  ...
  if (value == "$ptext") {  // par text without punc
    var text = parCopyText($(pidify(outData.pid)));
    if (text.substring(text.length - 1) == ".") {
      return text.substring(0, text.length - 1);
    } else {
      return text;
  ...
  if (value == "$medText") {
    return a.innerText.replace(a.medName, "").substring(3);
  }
...
function popQuestionCallback(q) {
  ...
  onChange(q.id);
  doOnChanges('popQuestionCallback');
}
...
function addHmHist(qid, hmid, proc, date, results) {
  var q = questions[qid];
  if (q.hms == null) {
    q.hms = {};
  }
...
function addMedByQid(qid, name, amt, freq, route, length, asNeeded, withMeals, disp) {
  addMed("q_" + qid + "o", name, amt, freq, route, length, asNeeded, withMeals, disp);
}
function addAllergyByData(qid, agent, reactions) {
  var q = questions[qid];
  var r = eval(reactions);
...
function getPar(pid, desc) {  // for clones, pid + instance e.g. 501+1
  ...
  pushAction("getPar('" + pid + "','" + desc + "')", "Get \"" + desc + "\"");
  addToUsed(pidi.pid, setReqPids(pidi.pid));
  sendGetParRequest(pid, null);
}
...
function selectHms(qid, hmids) {
  ...
  for (var i = 0; i < hmids.length; i++) {
    ...
    var date = (hm.date) ? hm.date + ": " : "";
    h.push(date + dashJoin(hm.results));
  }

...
class Procedures {
  ...
  static function getAllProcOnly($cid) {
    $recs = Proc::fetchAllProcOnly($cid);
    $recs = Proc::summarizeResults($recs);
    if ($recs)
      Rec::sort($recs, new RecSort('-date', 'Ipc.name'));
    return $recs;
  ...
  static function getAllPlanOfCare($cid) {
    $recs = Proc::fetchAllPlanOfCare($cid);
    if ($recs)
      Rec::sort($recs, new RecSort('-date'));
    return $recs;
  }
  static function getReasonForVisit($cid) {
    $recs = Proc::fetchReasonsForVisit($cid);
    if ($recs) {
      Rec::sort($recs, new RecSort('-date'));
      return reset($recs)/*most recent*/;
    }
  }
  static function getEncounters($cid, $lastOnly = false) {
    $recs = Proc::fetchEncounters($cid);
    if ($recs) {
      return $lastOnly ? array(reset($recs)) : $recs;
    }
  }
  static function getSmokingStatus($cid) {
    $recs = Proc::fetchAllSmoking($cid);
    if ($recs) {
      Rec::sort($recs, new RecSort('-date'));
      return reset($recs)/*most recent*/;
    }
  }
  static function getAllFuncStatus($cid) {
    $recs = Proc::fetchAllFuncStatus($cid);
    if ($recs)
      Rec::sort($recs, new RecSort('-date', 'Ipc.name'));
    return $recs;
  }
  static function deleteForHl7Inbox($id, $ugid, $cid) {
    Proc::deleteForHl7Inbox($id, $ugid, $cid);
  }
  /**
  ...
  static function savePanel($obj) {
    ...
    try {
      ...
      $rec->save($login->userGroupId);
      $proc = self::get($rec->procId);
      $results = ProcResult::revive_asPanelResults($obj, $proc);
      ProcResult::saveAll($results);
      if (isset($obj->Order))
        OrderEntry::receivedByProc($obj->Order->trackItemId, $proc->procId);
      Dao::commit();
    } catch (Exception $e) {
...
class Proc extends ProcRec {
  ...
  public function getJsonFilters() {
    return array(
      'date' => JsonFilter::editableDateTimeApprox(),
      'dateTo' => JsonFilter::editableDateTimeApprox(),
  ...
  static function fetchAllProcOnly($cid) {
    $c = new static();
    $c->clientId = $cid;
    $c->Ipc = Ipc::asRequired_noAdminOrLab();
    $c->Provider = Provider::asOptionalJoin();
    $c->Facility = FacilityAddress::asOptionalJoin();
    $c->ProcResults = ProcResult::asOptionalJoin();
  ...
  static function fetchAllPlanOfCare($cid) {
    $c = new static();
    $c->clientId = $cid;
    $c->Ipc = Ipc::asRequired_planOfCare();
    $c->Provider = Provider::asOptionalJoin();
    $c->Facility = FacilityAddress::asOptionalJoin();
    $c->ProcResults = ProcResult::asOptionalJoin();
    return self::fetchAllBy($c);
  ...
  static function fetchAllSmoking($cid) {
    $c = new static();
    $c->clientId = $cid;
    $c->Ipc = Ipc::asRequired_smoking();
    return self::fetchAllBy($c);
  }
  static function fetchAllFuncStatus($cid) {
    $c = new static();
    $c->clientId = $cid;
    $c->Ipc = Ipc::asRequired_funcStatus();
    $c->ProcResults = ProcResult::asOptionalJoin();
    return self::fetchAllBy($c);
  }
  static function fetchAllForIpc($cid, $ipc) {
    $c = new static();
    $c->clientId = $cid;
    $c->ipc = $ipc;
  ...
  static function fetchEncounters($cid) {
    $c = new static();
    $c->clientId = $cid;
    $c->ipc = Ipcs::OfficeVisit;
    //$c->setDateCriteria($dos);
...
class ProcResult extends ProcResultRec {
  ...
  static function revive_asPanelResults($obj, $proc) {
  ...
  static function revive_asPanelResult($obj, $procId, $seq) {
    $me = new static($obj);
    $me->procId = $procId;
    $me->seq = $seq;
    return $me;
...
class ProcResult_Hist extends ProcResult implements ReadOnly {
  ...
  static function fetch_byResult($rec) {
    return static::fetch($rec->clientId, $rec->ipc);
  }
}

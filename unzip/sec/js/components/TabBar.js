...
function TabBar(p, titles, tabCaptions, selIndex, flickerTrs) {
  ...
  this.bar = p.getTagsByClass('tabbar', 'DIV')[0];
  //this.panels = $$$$("tabpanel", p, "DIV");
  this.panels = p.getTagsByClass("tabpanel", "DIV");
  this.titles = titles;
  this.flickers = flickerTrs;
  this.tabCaptions = tabCaptions || titles;
...
TabBar.prototype = {
  ...
  onSelect:function(callback) {
    ...
    callback(this.selIndex, this.panels[this.selIndex]);
  },
  /*
   * @return int
   */
  ...
  _appendTitles:function(th) {
    ...
    for (var i = 0; i < this.titles.length; i++) {
      var h2 = Html.H2.create(this.titles[i]);
      this._h2s.push(h2);
      th.appendChild(h2);
    }
    return th;
  ...
  _appendTabs:function(td) {
    ...
    for (var i = 0; i < this.tabCaptions.length; i++) {
      if (i > 0 && this.tabCaptions[i]) 
        td.appendChild(Html.Span.create().html('&nbsp;&nbsp;&bull;&nbsp;&nbsp;'));
      with ({index:i}) { 
        var self = this;
...
TabBar.construct = function(parent, panelCount) {
  createDivIn(parent, 'tabbar');
  var tabpanels = createDivIn(parent, 'tabpanels');
  var panels = [];
  for (var i = 0; i < panelCount; i++) 
    panels.push(createDivIn(tabpanels, 'tabpanel'));
  return panels;
}; 
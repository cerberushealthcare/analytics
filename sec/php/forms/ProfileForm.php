...
class ProfileForm extends Form {
  ...
  private function buildHelpers() {
    ...
    $this->htmlPracticeAddress = $this->buildAddress($this->practice->address) . "<br/>" . $tz[$this->practice->estAdjust];
  ...
  private function buildBillInfo($billInfo) {
    ...
    $a = new Address0(null, null, null, null, $billInfo->address1, $billInfo->address2, null, $billInfo->city, $billInfo->state, $billInfo->zip, $billInfo->country, $billInfo->phone, null, null, null, null, null, null, null, null);
  ...
  private function buildSubscription($me) {
    global $login;
    if ($me->subscription == User0::SUBSCRIPTION_TRIAL) {
      $h = ProfileForm::buildTrial($me);
    } else if ($me->subscription == User0::SUBSCRIPTION_FREE) {
      $h = "<em>Free Subscription</em>";
    } else if ($me->subscription == User0::SUBSCRIPTION_INVOICE) {
...
//      $h .= "<a href='registerCard.php'>Change plan</a>";
    ...
    $h .= "<div><em>E-Prescribe:</em> " . ($login->isErx() ? 'Yes (NewCrop)' : 'No') . "</div>";
    return $h;
  }
  ...
  private function buildTrial($me) {
    global $login;
    $h = "<div";
    if ($login->User->isOnTrial() && $login->daysLeft < 25) {
      $h .= " class='red'>";
    } else {
      $h .= ">";
    }
    $h .= "<em>Free Trial (";
    $h .= $this->plural($login->daysLeft, "day");
    $h .= " left)</em></div>";
    if ($login->User->isOnTrial()) {
      if ($login->daysLeft < 7) {
        $h .= "<p class='bigfont'>";
        $h .= "<a href='registerCard.php'>Activate now</a> to convert to a paid subscription.";
        $h .= "</p>";
      } else {
        $h .= "<p class='bigfont'>";
        $h .= "<a href='registerCard.php'>Click here to activate your account</a> at any time before the countdown ends to convert to a paid subscription.";
        $h .= "</p>";
      }
    }
    return $h; 

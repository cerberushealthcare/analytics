...
if (isset($_SERVER['HTTP_HOST'])) {
  ...
  header("Location: ../index.php");
	...
	$res = query("SELECT user_id, name, address1, address2, city, state, zip, card_number, exp_month, exp_year, next_bill_date, balance, start_bill_date, bill_code FROM billinfo WHERE next_bill_date <='" . date('Y-m-d', strtotime("now")) . "'");
	...
	while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
	   ...
	   if ($myHost == "prod") {
	           ...
	           try {
		      if ($billamt >= .01)
	                 $result = OldBillingDao::chargeUser($bill);
	...
	$to       = "pstewart@clicktate.com, gkowalik@clicktate.com";
...
//	$to       = "pstewart@clicktate.com, gkowalik@clicktate.com";
	...
	$headers .= 'Reply-To: info@clicktate.com' . "\r\n";
	$headers .= 'Return-Path: info@clicktate.com' . "\r\n";
	$headers .= 'Bcc: pstewart@clicktate.com, gkowalik@clicktate.com' . "\r\n";
	$headers .= 'MIME-Version: 1.0' . "\r\n";

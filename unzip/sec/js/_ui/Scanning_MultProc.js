  ...
  EntryForm:function(self) {
    return {
      ...
      init:function() {
        ...
        self.Procs = self.$('procs').extend(self._Procs);
      },
      onbeforeload:function(rec) {
        self.setOrderClient(rec && rec.clientId);
      },
      onload:function() {
        if (self.rec && self.rec.Proc)
          self.Procs.load([self.rec.Proc]);
      },
      draw:function() {
        self.showRecon();
        self.showScanType();
      ...
      _Procs:function(self) {
        return {
          //
          init:function() {
            self.hide();
          },
          show:self.show.bind(self).append(function() {
            //if (self.Forms.length == 0)
            //  self.addForm();
          }),
          load:function(recs) {
            self.reset();
            Array.each(recs, function(rec) {
              self.addForm(rec);
            })
            return self.show();
          },
          getRecs:function() {
            ...
            self.Forms.each(function(Form) {
              recs.push(Form.getRecord());
            })
            return recs;
          },
          reconTo:function(order) {
          ...
          addForm:function(rec) {
            var Form = Html.UlEntry.create(self).extend(self._Form).load(rec);
          ...
          _Form:function(self) {
            return {
              init:function() {
                self.build('first70')
                .line()
                  .l('Test/Proc').pick('ipc', 'Ipc', IpcPicker)
                  .append(self.Quick = Html.Anchor.create('ml5', 'Quick Result...', self.quick_onclick))
              },
              onbeforeload:function(rec) {
                if (rec == null)
                  return Proc_Scan.asNew();
              },
              onload:function() {
                self.Quick.showIf(self.rec.isNew());
              },
              quick_onclick:function() {
                self.Pop.pop(self.rec, function(text) {
                  if (text)
                    self.Quick.setText('Quick Result: ' + text);
                })
              },
              Pop:Html.SingletonPop.aug({
                create:function() {
                  return Html.Pop.create('Quick Result', 600).extend(function(self) {
                    return {
                      ...
                      init:function() {
                        ...
                        Html.Pop.CmdBar.create(self).okCancel();
                      },
                      onshow:function(rec, callback) {
                        self.rec = rec;
                        self.callback = callback;
                        self.Form.load(rec).focus('value');
                      },
                      ok_onclick:function() {
                        self.rec = self.Form.getRecord();
                        self.close();
                      ...
                      getText:function() {
                        ...
                        if (r.interpretCode)
                          a.push(C_ProcResult.INTERPRET_CODES[r.interpretCode]);
                        if (a.length == 0 && ! String.isBlank(a.rcomments))
                          a.push(a.rcomments);
...
ScanIndex = Object.Rec.extend({
  onload:function() {
    ...
    this.Proc = Proc_Scan.revive(this.Proc);
  },
  ajax:function(worker) {
    var self = this;
...
Proc_Scan = Object.Rec.extend({
  isNew:function() {
    return this.procId == null;
  },
  //
  asNew:function() {
    return Proc_Scan.revive({});
  }
})
Proc_Scans = Object.RecArray.of(Proc_Scan, function() {
  //
})
...
class NewCrop {
  ...
  public function __construct() {
    ...
    switch ($this->env) {
      case Env::ENV_PRODUCTION:
        ...
        break;
      case Env::ENV_PAPYRUS_PROD:
        $this->credentials = new Credentials('clicktate', '34f1aabe-d51a-4f4c-b505-234edfa840c6', 'c1015a3b-1196-4a86-8262-b0c57052f62b'); 
        $this->credentials->setIds("YP$login->userGroupId", 1, $login->userGroupId, $this->users['lp']->userId);
        $this->clickThruUrl = NewCrop::NC_PROD_CT_URL; 
        $this->wsHost = NewCrop::NC_PROD_WSHOST;
        break;
   ...
   */
  public function pullAcctStatusDetails($usecache = false) {
    $statuses = StatusPharmRequest::makeStatusesArray(); 
    foreach ($statuses as $status => &$recs)
      $recs = $this->pullAcctStatusDetail($status, $usecache);
    return $statuses;
  }
  /**
   ...
   *      </xs:element>
   */
  public function pullAcctStatusDetail($sectionType, $usecache = false) {
    global $login;
    $return = null;
    ...
    $response = $usecache ? NewCrop_Cache::fetchResponse($login->userGroupId, $param) : null;
    if ($response == null) {
      $soap = $this->createSoap($this->getUpdate1WsUrl());
      $response = $soap->GetAccountStatusDetail($param);
      NewCrop_Cache::saveResponse($login->userGroupId, $param, $response);
    }
   ...
   *      </RenewalSummaryV2>
   */
  public function pullRenewalRequests($lpid = null, $usecache = false) {
    global $login;
    if ($lpid == null)
   ...
   */
  public function pullAllRenewalRequests($usecache = false) {
    $all = array();
    foreach ($this->users['lps'] as $lp) {
      $reqs =  $this->pullRenewalRequests($lp->userId, $usecache);
      if ($reqs) 
  ...
  //
  public function /*DailyMeaningFulUseReport[]*/pullDailyMuReport($date = null, $lpid = null) {
    global $login;
    if ($lpid == null)
      $lpid = $this->credentials->lpId;
    if ($date == null)
      $date = nowYYYYMMDD();
    $results = DailyMeaningfulUseReport::fetch($this->wsHost, $this->credentials, $lpid, $date);
    return $results;
  }
  /**
   * @deprecated
  ...
  private function getEnv() {
    global $login;
    if ($login->env == Env::ENV_PRODUCTION) {
      if ($login->User->isOnTrial())
        return static::ENV_PROD_TRIAL;
      if ($login->demo)
        return static::ENV_PROD_DEMO;
    }
    return $login->env;
  }
}
 ...
 * NewCrop Cache 
 */
class NewCrop_Cache extends SqlRec implements CompositePk, NoAudit {
  //
  public $userGroupId;
  public $hashParam; /*sha1 SOAP param*/
  public $expires;
  public $response; /*json_encoded SOAP response*/
  //
  static function /*obj*/fetchResponse($ugid, /*obj*/$param) {
    $hash = static::getHashParam($param);
    $me = static::fetch($ugid, $hash);
    if ($me && ! $me->isExpired())
      return $me->getResponse();
  }
  static function saveResponse($ugid, /*obj*/$param, /*obj*/$response) {
    $me = new static();
    $me->userGroupId = $ugid; 
    $me->hashParam = static::getHashParam($param);
  ...
  public function setExpires($minutes = 15) {
    $this->expires = mktime(date('G'), date('i') + $minutes, 0);
  }
  ...
  public function getSqlTable() {
    return 'newcrop_cache';
  }
  public function getPkFieldCount() {
    return 2;
  }
  //
  static function fetch($ugid, $hash) {
    $c = new static();
    $c->userGroupId = $ugid; 
    $c->hashParam = $hash;
    return static::fetchOneBy($c);
  }
  protected static function getHashParam(/*obj*/$param) {
